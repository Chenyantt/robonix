...
CONF
  ...
  basic_step="${ROBONIX_WEBOTS_FAST_BASIC_TIME_STEP:-62}"
  ...
  if [ -f "$source_robot" ] && [ -n "$camera_rate" ] && [ "$camera_rate" != "0" ]; then
    ...
    perl -0pi -e '
      s#(<device reference="(?:Astra(?:_front|_back)? rgb|Astra(?:_front|_back)? depth)" type="(?:Camera|RangeFinder)">\s*<ros>\s*)(?!<updateRate>)#$1<updateRate>$ENV{ROBONIX_WEBOTS_FAST_CAMERA_UPDATE_RATE}</updateRate>\n                #g
    ' "$fast_robot"
...
ROBONIX_WEBOTS_ROBOT="${ROBONIX_WEBOTS_ROBOT:-ranger_mini_v3_webots.urdf}"

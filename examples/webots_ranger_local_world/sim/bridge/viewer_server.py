...
LOCAL_WORLD_RESOURCE_ROOTS = (
    "/colcon_ws/install/eaios_webots/share/eaios_webots/worlds",
)
...
class ViewerHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if self.path.split("?", 1)[0] in ("/", "/index.html"):
            ...
            self.proxy_stream()
        elif self.path.startswith("/colcon_ws/"):
            self.serve_local_world_resource()
    ...
    def serve_local_world_resource(self) -> None:
        """Serve local world assets referenced by OBJ/MTL files in W3D mode."""
        path = self.path.split("?", 1)[0]
        candidate = os.path.realpath(urllib.request.url2pathname(path))
        for root in LOCAL_WORLD_RESOURCE_ROOTS:
            real_root = os.path.realpath(root)
            if candidate.startswith(real_root + os.sep) and os.path.isfile(candidate):
                try:
                    with open(candidate, "rb") as source:
                    ...
                    self.send_error(500, f"Local world resource unavailable: {error}")
                    return
                self.send_content(content, os.path.basename(candidate), 86400)
                return
        self.send_error(404, "Local world resource not found")

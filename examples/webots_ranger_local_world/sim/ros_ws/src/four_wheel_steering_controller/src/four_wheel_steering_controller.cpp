...
inline void use_shortest_steering_angle(double & steering, double & wheel_velocity)
{
...
controller_interface::return_type FourWheelSteeringController::updateCommand(const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  
  {
    ...
    {
      const auto compute_wheel_command =
        [this, &curr_cmd_twist](double wheel_x, double wheel_y,
          double & wheel_velocity, double & steering)
        {
          const double velocity_x = curr_cmd_twist->lin_x - curr_cmd_twist->ang * wheel_y;
          const double velocity_y = curr_cmd_twist->lin_y + curr_cmd_twist->ang * wheel_x;
          wheel_velocity = std::hypot(velocity_x, velocity_y) / wheel_radius_;
          steering = atan2(velocity_y, velocity_x);
          use_shortest_steering_angle(steering, wheel_velocity);
        };

      const double half_wheel_base = wheel_base_ / 2.0;
      const double half_steering_track = steering_track / 2.0;
      compute_wheel_command(
        half_wheel_base, half_steering_track, vel_left_front, front_left_steering);
      compute_wheel_command(
        half_wheel_base, -half_steering_track, vel_right_front, front_right_steering);
      compute_wheel_command(
        -half_wheel_base, half_steering_track, vel_left_rear, rear_left_steering);
      compute_wheel_command(
        -half_wheel_base, -half_steering_track, vel_right_rear, rear_right_steering);
    }

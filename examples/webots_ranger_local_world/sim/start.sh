...
# Bring up the Ranger Webots sim container. Run this BEFORE `rbnx boot`
...
export ROBONIX_SIM_CONTAINER="${ROBONIX_SIM_CONTAINER:-robonix_ranger_local_world_sim}"
export ROBONIX_SIM_PROJECT="${ROBONIX_SIM_PROJECT:-robonix_ranger_local_world_sim}"
...
export ROBONIX_SIM_ROS_BASE_IMAGE="${ROBONIX_SIM_ROS_BASE_IMAGE:-robonix_tiago_sim-sim:latest}"
robonix_ensure_local_base_image "$ROBONIX_SIM_ROS_BASE_IMAGE" "robonix_tiago_sim-sim:latest"
cd "$SCRIPT_DIR"
...
export ROBONIX_WEBOTS_WORLD="${ROBONIX_WEBOTS_WORLD:-test_world_ranger.wbt}"
export ROBONIX_WEBOTS_ROBOT="${ROBONIX_WEBOTS_ROBOT:-ranger_mini_v3_webots.urdf}"
...
echo "[sim/start] fast profile: ${ROBONIX_WEBOTS_FAST_PROFILE:-1} (basicTimeStep=${ROBONIX_WEBOTS_FAST_BASIC_TIME_STEP:-62}ms, lidar=${ROBONIX_WEBOTS_FAST_LIDAR_RESOLUTION:-330}, camera=${ROBONIX_WEBOTS_FAST_CAMERA_UPDATE_RATE:-5}Hz, control=${ROBONIX_WEBOTS_FAST_CONTROL_UPDATE_RATE:-16}Hz)"

maybe_enable_wsl_proxy_for_webots_assets() {
  [[ "${ROBONIX_AUTO_WSL_PROXY:-1}" = "1" ]] || return 0
  ...
  local windows_ip proxy_addr status_code
  windows_ip=$(ip route show default 0.0.0.0/0 | awk 'NR==1 {print $3}')
  [[ -n "$windows_ip" ]] || return 0

  proxy_addr="http://${windows_ip}:${WSL_PROXY_PORT:-7890}"
  status_code=$(HTTP_PROXY="$proxy_addr" HTTPS_PROXY="$proxy_addr" ALL_PROXY="$proxy_addr" \
    curl --silent --show-error --location --max-time 8 \
      --output /dev/null --write-out "%{http_code}" \
      https://raw.githubusercontent.com/cyberbotics/webots/R2025a/projects/objects/floors/protos/Floor.proto \
    2>/dev/null || true)
  case "$status_code" in
    2*|3*)
      ...
      echo "[sim/start] WSL proxy enabled for Webots asset downloads: $proxy_addr"
      ;;
  esac
}

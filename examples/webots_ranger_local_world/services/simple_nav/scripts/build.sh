...
if command -v rbnx >/dev/null 2>&1; then
    ...
    if docker ps --format '{{.Names}}' | grep -qx robonix_ranger_local_world_sim; then
      _IDL="/robonix_pkgs/$(basename "$(dirname "$PKG")")/$(basename "$PKG")/rbnx-build/codegen/ros2_idl"
      docker exec robonix_ranger_local_world_sim bash -lc "source /opt/ros/humble/setup.bash && cd $_IDL && colcon build"
    else

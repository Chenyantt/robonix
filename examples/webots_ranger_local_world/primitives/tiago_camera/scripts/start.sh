...
SIM_CT="${ROBONIX_SIM_CONTAINER:-robonix_ranger_local_world_sim}"
...
# Webots-compat static TFs: the Ranger PROTO exposes its front RGB-D links as
# "Astra_front rgb/depth", while the camera primitive and scene stack use the
# optical frame names below.
docker exec -d "$SIM_CT" bash -lc "
    ...
    exec ros2 run tf2_ros static_transform_publisher \
        ...
        --frame-id 'Astra_front rgb' --child-frame-id head_front_camera_rgb_optical_frame
" &>/dev/null
docker exec -d "$SIM_CT" bash -lc "
    set +u
    source /opt/ros/humble/setup.bash >/dev/null
    ...
    exec ros2 run tf2_ros static_transform_publisher \
        --x 0 --y 0 --z 0 --yaw 0 --pitch 0 --roll 0 \
        --frame-id 'Astra_front depth' --child-frame-id head_front_camera_depth_optical_frame
" &>/dev/null
